...
<body><center><br>
	...
	<table border=0>
		<tr>
			<td align=right>
				[검색 총 개수] : ${requestScope.discontentListAllCnt}&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="javascript:goDiscontentRegForm();">[새 글쓰기]</a>
		...
		<tr>
			<td>
				<table border=0 class="discontentList tbcss2" cellpadding=5 cellspacing=0>
					<tr><th>번호<th>제목<th>글쓴이<th>등록일<th>조회수
					<!--****************************************************************-->
					<!--HttpServletRequest 객체에 boardList 라는 키값으로 저장된********-->
					<!--ArrayList<HashMap<String,String>> 객체를 꺼내고*****************-->
					<!--ArrayList<HashMap<String,String>> 객체 안의*********************-->
					<!--HashMap<String,String> 을 꺼내고 HashMap<String,String> 객체안의-->
					<!--키값에 대응하는 문자열을 꺼내서 출력하기************************-->
					<!--****************************************************************-->
					<!--HttpServletRequest 객체에 boardList 라는 키값으로 저장된********-->
					<!--ArrayList<HashMap<String,String>> 객체안의 <HashMap<String,String> 들을-->
					<!--자바 지역변수 board 에 1개씩 저장하고 반복문 안으로 들어가서-->
					<!--달러{지역변수board.HashMap객체키값명} 으로 표현하고 있다-->
					<!--반복문 돌때 마다 자바 지역변수 board 에는 n 번째 <HashMap<String,String> 객체가 저장된다-->
					<!--반복문 돌때마다 LoopTagStatus 객체의 index 라는 속성변수안의 데이터를 꺼내어-->
					<!--출력한다. 출력 시 형식은 EL 로 달러{loopTagStatus} 로 한다-->
					<!--반복문 돌때 마다 LoopTagStatus 객체의 count 라는 속성변수안의 데이터는 0부터-->
					<!--시작해서 1씩 증가 하면서 반복문 안에서 표현된다-->
					<!--현재 LoopTagStatus 객체의 메위주는 loopTagStatus 라는 지역 변수에 저장되어 있다-->
					<!--바로 varStatus="loopTagStatus" 에 선언된 지역변수이다-->
					<!--****************************************************************-->
					<c:forEach items="${requestScope.discontentList}" var="discontent" varStatus="loopTagStatus"> <!--boardList는 BoardListFormAction에 request.setAttribute("boardList", boardList);에서 "boardList" 요고다--> <!--board 는 지역변수-->
	...
	<!--*********************************************************************-->
	<form name="discontentRegForm" method="post" action="/support/discontentRegForm.do">
	</form>
	<!--*************************************************************************-->
